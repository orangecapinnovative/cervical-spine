...
Spinal.prototype.start = function(cb) {
  ...
  app.use(morgan(
    (tokens, req, res) => {
      const serviceName = req.body && req.body.name ? req.body.name : ''
      const displayServiceName = `${that.namespace}.${serviceName}`;
      const context = tracer.getContext()
      const projectId = process.env.GCP_PROJECT || process.env.GOOGLE_CLOUD_PROJECT || 'takemetour-176217'
      ...
      const logEntry = {
        ...
        httpRequest: {
          ...
          requestUrl: displayServiceName,
          status: parseInt(tokens.status(req, res), 10),
          responseSize: tokens.res(req, res, 'content-length'),
          userAgent: req.headers['user-agent'],
          remoteIp: req.ip || req.connection.remoteAddress
        },
        message: `${tokens.method(req, res)} ${displayServiceName} ${tokens.status(req, res)}`,
        'logging.googleapis.com/labels': {
          service_name: serviceName,
          service_namespace: that.namespace,
      ...
      if (context.traceId) {
        logEntry['logging.googleapis.com/trace'] = `projects/${projectId}/traces/${context.traceId}`
      }
      if (context.spanId) {
        logEntry['logging.googleapis.com/spanId'] = context.spanId
      }
    ...
    {
      skip: (req) => {
        if (req.body && req.body.name === '_ping') return true
        return false;
      }
    }
  )
  ...
  that.server = app.listen(that.port, function() {
    ...
    cb && cb()
    that.emit('listening')
    logger.info(`Service ${that.namespace} started`, {
      'logging.googleapis.com/labels': {
        service_namespace: that.namespace,
        app_env: process.env.NODE_ENV,
      }
  ...
  process.on('SIGTERM', function onSigterm () {
    ...
    logger.info('Got SIGTERM. Graceful shutdown start', { timestamp: new Date().toISOString() })
  ...
  .then(function(response) {
    if (!response)
      return axios.post('http://' + host +':' + port, {
        ...
        data: arg,
        options: options
      }, Object.assign(axiosOptions, { headers: tracer.getTraceContextHeaders() })).then((resp) => {
...
Spinal.prototype.worker = function(name, fn) {
  logger.warn('Job queue is not currently supported in cervical-spine')
}
